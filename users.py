...
def signup(user_request : schemas.CreateUser, db:Session = Depends(database.get_db), checkrole:dict = Depends(check_role)):
...
def update_user(id: int, update:schemas.CreateUser, db:Session = Depends(database.get_db),checkrole:dict = Depends(check_role)):
    upd_user = db.query(models.User).filter(models.User.id == id).first()
    if not upd_user:
